# Forever Space Stable 1.41 — Item DB Reference
...
Current item data truth is the separated database scripts:
...
`item_handler.gd` remains the runtime/API handler. Do not treat the old monolithic item dictionary as current truth.

## Validation Facts

Split V2 preserved the original source set:

```text
Original source item entries: 122
Split item entries: 122
Missing IDs: 0
Extra IDs: 0
Duplicate IDs: 0
Confirmed base resources: iron, cobalt, nickel, gold, credits
```

Current active builder-loaded index:

```text
Active builder-loaded item holders: 11
Active builder-loaded item call names: 170
Active unique item call names: 170
Active duplicate call names: 0
```
...
Control/items/item_db_builder.gd
Control/items/item_db_*.gd
Control/Inventory5.gd   # optional if item_db API patch is already applied
```
...
so `main_mode.gd` can continue using:
...
## Builder-Loaded Active Item Call Names

## `item_db_modules.gd` — 2 active call names
| Item call name |
|---|
| `scan_module_mk1` |
| `drone_controller_mk1` |

## `item_db_consumables.gd` — 21 active call names
| Item call name |
|---|
| `repair_kit` |
| `shield_patch_cell` |
...
## `item_db_event_items.gd` — 2 active call names
| Item call name |
|---|
| `data_chip_empty` |
| `data_chip_full` |

## `item_db_drones.gd` — 5 active call names
| Item call name |
|---|
| `roamer_drone_mk1` |
| `scout_drone` |
...
## `item_db_base_resources.gd` — 5 active call names
| Item call name |
|---|
| `iron` |
| `cobalt` |
...
## `item_db_space_materials.gd` — 16 active call names
| Item call name |
|---|
| `silicate_dust` |
| `carbon_compounds` |
...
## `item_db_parts.gd` — 1 active call names
| Item call name |
|---|
| `navigation_relay_coupler` |

## `item_db_ammo.gd` — 4 active call names
| Item call name |
|---|
...
## `item_db_blueprints.gd` — 51 active call names
| Item call name |
|---|
...
## `item_db_weapons.gd` — 48 active call names
| Item call name |
|---|
...
## `item_db_shields.gd` — 15 active call names
| Item call name |
|---|
...
## Present But Not Builder-Loaded Rule

Some legacy/test/source-reference slices exist in the workspace but are not merged by `item_db_builder.gd`.

Treat these as non-active unless the builder is intentionally updated:
...
The active builder-loaded truth has no duplicate call names. Duplicate names only appear if non-active legacy/test/reference slices are included in a scan.
...
[ ] Confirm item_handler.has_item("iron") returns true.
[ ] Confirm item_handler.has_item("cobalt") returns true.
[ ] Confirm item_handler.has_item("nickel") returns true.
